...
def generate_config_txt(wafer_num: int, config: str, physical_filepath: str, output_filepath: str):
    ...
    for idx in range(0, wafer_num):
        ...
        physical_filename = f"{config}_{idx_padded}_physical_network.txt"
        physical_file = os.path.join(physical_filepath, physical_filename)
        ...
        for i, line in enumerate(config_content):
            if i == 5:  # 第六行（索引为5）
                new_content.append(f"TOPOLOGY_FILE {physical_file}\n")
            else:
...
if __name__ == "__main__":
    ...
    output_filepath = f"/data/login_home/lijinxi/error_tolerance/output/config/"
