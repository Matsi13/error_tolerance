...
solution_distance calculate_distance(Wafer& first, Wafer& second){
    ...
    result.on_die_mem_capacity = first_on_die_mem_capacity - second_on_die_mem_capacity;
...
void Possible_optimal(list<Wafer> solutions, Wafer& optimal, Arch_error& error, list<Wafer>& possible_optimals){
    ...
    for(auto idx = solutions.begin(); idx != solutions.end(); idx++){

        Wafer possible_solution = *idx;
        ...
        float on_die_mem_capacity_max = (1 + error.on_die_mem_capacity_positive) * optimal.get_on_die_mem_capacity();
        float on_die_mem_capacity_min = (1 + error.on_die_mem_capacity_negative) * optimal.get_on_die_mem_capacity();
        ...
        float on_die_mem_capacity = possible_solution.get_on_die_mem_capacity();
        ...
        bool is_within_on_die_mem_capacity = (on_die_mem_capacity >= on_die_mem_capacity_min) && (on_die_mem_capacity <= on_die_mem_capacity_max);
        ...
        bool is_within_error = is_within_TFLOPS || is_within_SRAM_capacity || is_within_on_die_mem_capacity || is_within_memory_bandwidth || is_within_communication_bandwidth;
        
        if(is_within_error){
            possible_optimals.push_back(possible_solution);
        }
